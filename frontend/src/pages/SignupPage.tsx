...
export default function SignupPage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        await initCsrf();
        const res = await api.get('/api/signup/verify', { params: { token } });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/api/signup/complete', { token, password, password_confirmation: passwordConfirmation });
      setMessage('パスワードを設定しました。ログインできます。');
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 rounded border">
      ...
      <form className="space-y-4 mt-4" onSubmit={onSubmit}>
        ...
        <button className="px-4 py-2 rounded bg-blue-600 text-white">設定</button>
      </form>
    </div>
  );
}
